  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Try to fetch shifts (handle gracefully if fails)
        try {
          ...
          console.log("Shifts endpoint not available yet");
          setShifts([]);
        }
        
        // Try to fetch notifications (handle gracefully if fails)
        try {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {isEditing ? (
                    <>
                      ...
                      <button 
                        onClick={saveProfile}
                        disabled={saving}
                        className="w-full mt-4 bg-gradient-to-r from-amber-600 to-amber-500 text-black py-2 rounded-sm font-sans font-bold text-[11px] tracking-[0.2em] uppercase flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-amber-500/20 transition-all"
                      >
                        {saving ? (
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-black/40 backdrop-blur-xl border border-amber-500/20 rounded-sm p-6">
                ...
                {notifications.length > 0 ? (
                  notifications.map((n, idx) => (
                    <div key={idx} className="bg-black/40 border border-amber-500/10 rounded-sm p-3 mb-2">
              ...
              <div className="bg-black/40 backdrop-blur-xl border border-amber-500/20 rounded-sm p-6">
                ...
                {shifts.length > 0 ? (
                  shifts.map((s, idx) => (
                    <div key={idx} className="flex justify-between items-center p-3 border-b border-amber-500/10 last:border-0">
                      <div>
